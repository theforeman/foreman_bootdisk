...
Subnet image is similar to generic image, but chain-loading is done via TFTP
Smart Proxy assigned to Subnet of the host. The smart proxy must have
"Templates" module enabled and configured.

This image is generic for all hosts with provisioning NIC on that subnet.
